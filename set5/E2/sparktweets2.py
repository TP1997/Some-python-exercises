...
tweets=read_data("trump_tweets.csv")
...
def outputformat(row):
    ...
    for hour_pct in row[1]:
        res+="{:2.0f} ".format(round(hour_pct))

    return res

hour_dist = tweets.map(lambda kv_tuple: (kv_tuple[0], [kv_tuple[1]])).reduceByKey(lambda a,b: a+b).mapValues(map_hours).mapValues(reduce_hours) #.map(lambda x, y: (x, (y, 1)) )#.reduceByKey(add)
...
#res_dist.map(outputformat).saveAsTextFile("sparktweets.txt")

writable_data=res_dist.collect()

with open("sparktweetsr.txt", "w") as f:
